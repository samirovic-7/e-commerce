...
<?php include"./fixed_temp/header.php" ?>
                        ...
                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">
                                ...
                                <?php 
                                    ...
                                    foreach ($sql as $key) {
                                       ...
                                       <a href='Tablets.php?filter=$id' class='butt_filter'>
                                       $name
                                       </a>
                                       </div>
                                   </div>";
        ...
        if(isset($_GET['filter'])){
            $id= $_GET['filter'];
            $sql = $database->prepare("SELECT * FROM tables WHERE type_id=$id");
            $sql->execute();
            foreach ($sql as $key) {
                $id = $key['Table_id'];
                $name = $key['Table_name'];
...
?>
      ...
      table();
            ...
            if(isset($_POST['Add'])){
                $UpdateCard = $database->prepare("UPDATE tables SET Card=1 WHERE Card=0 AND Table_id=:id");
                $UpdateCard->bindParam("id",$_POST['Add']);
                $UpdateCard->execute();
            };
            
        if(isset($_POST['AddProductToFavour'])){
            $UpdateFav =$database->prepare("UPDATE tables SET Fav=1 WHERE Fav=0 AND Table_id=:id");
