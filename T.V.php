...
<?php include"./fixed_temp/header.php" ?>
                        ...
                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">
                                ...
                                <?php 
                                    ...
                                    foreach ($sql as $key) {
                                       ...
                                       <a href='T.V.php?filterType=$id' class='butt_filter'>
                                       $name
                                       </a>
                                       </div>
                                   </div>";
                        ...
                        size
                                ...
                                <?php 
                                    ...
                                    foreach ($sql as $key) {
                                       ...
                                       <div class='accordion-body'>
                                       <a href='T.V.php?filterSize=$id' class='butt_filter'>
                                       $name
                                       </a>
                                       </div>
                                   </div>";
        ...
        if(isset($_GET['filterType'])){
            ...
            $sql = $database->prepare("SELECT * FROM tv WHERE tv_type=$idType");
            $sql->execute();
            foreach ($sql as $key) {
                $id = $key['tv_id'];
                $name = $key['tv_name'];
...
<?php
        ...
        if(isset($_GET['filterSize'])){
            $idSize= $_GET['filterSize'];
            $sql = $database->prepare("SELECT * FROM tv WHERE tv_size=$idSize");
            $sql->execute();
            foreach ($sql as $key) {
                $id = $key['tv_id'];
                $name = $key['tv_name'];
...
?>
           ...
           tv();
            if(isset($_POST['Add'])){
                $UpdateCard = $database->prepare("UPDATE tv SET Card=1 WHERE Card=0 AND tv_id=:id");
                $UpdateCard->bindParam("id",$_POST['Add']);
                $UpdateCard->execute();
            };
            
        if(isset($_POST['AddProductToFavour'])){
            $UpdateFav =$database->prepare("UPDATE tv SET Fav=1 WHERE Fav=0 AND tv_id=:id");
