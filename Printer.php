...
<?php include"./fixed_temp/header.php" ?>
                        ...
                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">
                                ...
                                <?php 
                                    ...
                                    foreach ($sql as $key) {
                                       ...
                                       <a href='Printer.php?filter=$id' class='butt_filter'>
                                       $name
                                       </a>
                                       </div>
                                   </div>";
        ...
        if(isset($_GET['filter'])){
            $id= $_GET['filter'];
            $sql = $database->prepare("SELECT * FROM printer WHERE Printer_type=$id");
            $sql->execute();
            foreach ($sql as $key) {
                $id = $key['Printer_id'];
...
?>
           ...
           printer();
            if(isset($_POST['Add'])){
                $UpdateCard = $database->prepare("UPDATE printer SET Card=1 WHERE Card=0 AND Printer_id=:id");
                $UpdateCard->bindParam("id",$_POST['Add']);
                $UpdateCard->execute();
            };
            if(isset($_POST['AddProductToFavour'])){
                $UpdateFav =$database->prepare("UPDATE printer SET Fav=1 WHERE Fav=0 AND Printer_id=:id");
