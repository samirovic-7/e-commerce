                                ...
                                <?php
                                    ...
                                    foreach ($sql as $key) {
                                       ...
                                       <div class='accordion-body'>
                                              ...
                                              <a href='Personal_computer(PC).php?filter=$id' class='butt_filter'>
                                              $name
                                              </a>
                                       </div>
                                   </div>";}
...
<?php
        ...
        if(isset($_GET['filter'])){
            $id= $_GET['filter'];
            $sql = $database->prepare("SELECT * FROM PC WHERE pc_type_id=$id");
            $sql->execute();
            foreach ($sql as $key) {
                $id = $key['pc_id'];
                $name = $key['pc_name'];
...
?>
           ...
           pc();
                if(isset($_POST['Add'])){
                    $UpdateCard = $database->prepare("UPDATE pc SET Card=1 WHERE Card=0 AND pc_id=:id");
                    $UpdateCard->bindParam("id",$_POST['Add']);
                    $UpdateCard->execute();
                };
                
        if(isset($_POST['AddProductToFavour'])){
            $UpdateFav =$database->prepare("UPDATE pc SET Fav=1 WHERE Fav=0 AND pc_id=:id");
