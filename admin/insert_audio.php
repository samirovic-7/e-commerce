...
<?php include"../fixed_temp/navAdmin.php" ?>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        ...
        <form class="row g-3 form" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-6 inplab ">
                ...
                <input type="text " class="form-control input " id="inputPhone " name="audio_name">
            </div>


            <div class="col-md-6 inplab ">
                <label for="inputselect " class="form-label ">The Type Of audio</label>

                <select class="form-select input " aria-label="Default select example "  name="type_id">
              <option hidden>Select Product</option>
              <?php
                $sql = $database->prepare("SELECT * FROM audio_type");
                $sql->execute();
                foreach ($sql as $key) {
                    $name = $key["Audio_type_name"];
                    $type_id = $key["Audio_type_id"];
                    echo"
            ...
            <div class="col-md-12 inplab ">
                ...
                <input type="text " class="form-control input " id="inputPhone " name="audio_title">
            </div>

            <div class="col-md-6 inplab ">
                <label for="inputPhone " class="form-label ">Price of product</label>
                <input type="number " class="form-control input " id="inputPhone" name="audio_price">
            </div>

            <div class="col-md-6 inplab ">
                <label for="inputPhone " class="form-label ">Rate of product</label>
                <input type="number " class="form-control input " id="inputPhone" name="audio_rate">
            </div>
...
if(isset($_POST['insert'])){
    ...
    $price = $_POST['audio_price'];
    $rate = $_POST['audio_rate'];
    $type = $_POST['type_id'];
    
    $image = $_FILES['image']['name'];
    $image_tmp = $_FILES['image']['tmp_name'];
    move_uploaded_file($image_tmp,"../img/Audio/$image");

    $sql=$database->prepare("INSERT INTO audio(Audio_name,audio_title,audio_price,audio_rate,Audio_type,audio_image) VALUES(:name,:title,:price,:rate,:type,:img)");
