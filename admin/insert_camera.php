...
<?php include"../fixed_temp/nav.php" ?>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        ...
        <form class="row g-3 form" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-6 inplab ">
                ...
                <input type="text " class="form-control input " id="inputPhone " name="camera_name">
            </div>
            ...
            <div class="col-md-6 inplab ">
              ...
              <?php
                $sql = $database->prepare("SELECT * FROM camera_type");
                $sql->execute();
                foreach ($sql as $key) {
                    $name = $key["Camera_type_name"];
                    $type_id = $key["Camera_type_id"];
                    echo"
            ...
            <div class="col-md-12 inplab ">
                ...
                <input type="text " class="form-control input " id="inputPhone " name="camera_title">
            </div>

            <div class="col-md-6 inplab ">
                <label for="inputPhone " class="form-label ">Price of product</label>
                <input type="number " class="form-control input " id="inputPhone" name="camera_price">
            </div>

            <div class="col-md-6 inplab ">
                <label for="inputPhone " class="form-label ">Rate of product</label>
                <input type="number " class="form-control input " id="inputPhone" name="camera_rate">
            </div>
...
if(isset($_POST['insert'])){
    ...
    $title = $_POST['camera_title'];
    $price = $_POST['camera_price'];
    $rate = $_POST['camera_rate'];
    $type = $_POST['type_id'];
    
    $image = $_FILES['image']['name'];
    $image_tmp = $_FILES['image']['tmp_name'];
    move_uploaded_file($image_tmp,"../img/camera/$image");

    $sql=$database->prepare("INSERT INTO camera(Camera_name,Camera_title,Camera_price,Camera_rate,Camera_type_id,Camera_image) VALUES(:name,:title,:price,:rate,:type,:img)");
