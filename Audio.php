...
<?php include"./fixed_temp/header.php" ?>
                                ...
                                <?php 
                                    ...
                                    foreach ($sql as $key) {
                                       ...
                                       <div class='accordion-body'>
                                              ...
                                              <a href='Audio.php?filter=$id' class='butt_filter'>
                                                 $name
                                                 </a>
                                       </div>
                                   </div>";
        ...
        if(isset($_GET['filter'])){
            $id= $_GET['filter'];
            $sql = $database->prepare("SELECT * FROM audio WHERE Audio_type=$id");
            $sql->execute();
            foreach ($sql as $key) {
                $id = $key['Audio_id'];
                $name = $key['Audio_name'];
...
?>
    ...
    <?php 
           ...
           Audio();
            ...
            if(isset($_POST['Add'])){
                $UpdateCard = $database->prepare("UPDATE audio SET Card=1 WHERE Card=0 AND Audio_id=:id");
        ...
        if(isset($_POST['AddProductToFavour'])){
            $UpdateFav =$database->prepare("UPDATE audio SET Fav=1 WHERE Fav=0 AND Audio_id=:id");
